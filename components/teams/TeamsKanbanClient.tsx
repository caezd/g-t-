...
function ClientHeaderRow({
    ...
    const allEntries = [
        ...(client.clients_mandats ?? []).flatMap(
            (m: any) => m.time_entries ?? []
        ),
...
function MandateRow({ mandat }: { mandat: any }) {
    ...
    return (
        <tr className="border-b last:border-b-0 text-sm">
            ...
            <td className="px-4 py-2">
                {FormatDecimalsToHours(totals.manager)}
            </td>
            <td className="px-4 py-2">
                {FormatDecimalsToHours(totals.assistant)}
            </td>
            <td className="px-4 py-2">
                {FormatDecimalsToHours(totals.helper)}
            </td>
            <td
                className={cn(
    ...
    const clientsInRange = useMemo(() => {
        ...
        return clients.map((c) => {
            const filteredMandats = (c.clients_mandats ?? []).map((m: any) => {
                const filteredEntries = (m.time_entries ?? []).filter(
