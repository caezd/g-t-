...
export function ClientPickerRow({ form }) {
  ...
  const label =
    clients.find((c) => c.id === selected)?.name ?? "Sélectionner un client";
          ...
          <FormItem className="flex flex-col col-span-2">
            ...
            <FormControl>
              <SearchableCombobox
                ...
                placeholder={
                  internal ? "Focus TDL / Interne" : "Sélectionner un client"
                }
          ...
          <FormItem className="flex flex-col items-start">
            ...
            <FormControl>
              <div className="h-10 px-3 flex items-center gap-3">
                ...
                />
                <span className="text-sm">FocusTDL / Interne</span>
              </div>
            </FormControl>
            <FormMessage />
          </FormItem>
...
function MandatePickerRow({ form }) {
  ...
  return (
    <FormField
      ...
      name="mandat_id"
      render={({ field }) => {
        const hasClient = internal || (clientId != null && clientId !== 0);
            ...
            <FormControl>
              <Select
                ...
                onValueChange={(value) => {
                  form.setValue(
                    "mandat_id",
                    value === NULL_SENTINEL ? null : Number(value),
                    {
                      shouldDirty: true,
                      shouldValidate: true,
                    },
                ...
                <SelectContent>
                  {mandates.map((m) => (
                    <SelectItem key={m.id} value={String(m.id)}>
                      {m.mandat_types.description ?? `Mandat #${m.id}`}
                    </SelectItem>
                  ))}
                  <SelectItem value={NULL_SENTINEL}>Hors mandat</SelectItem>
                </SelectContent>
              </Select>
            </FormControl>
            <FormMessage />
