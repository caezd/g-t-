...
const FormSchema = z.object({
  ...
  mandates: z.array(RowSchema).refine(
    (arr) => {
      ...
      return (
        new Set(active.map((x) => x.mandat_type_id)).size === active.length
      );
    },
...
export function EditClientDialog({
  ...
  async function onSubmit(values: FormValues) {
    ...
    const initialByType = new Map(initial.map((p) => [p.mandat_type_id, p]));
    ...
    for (const m of values.mandates) {
      ...
      if (m._delete) {
        if (init?.id && init.deleted_at == null) toSoftDelete.push(init.id);
        continue;
      }
    ...
    if (toInsert.length) {
      ...
      const { error } = await supabase.from("clients_mandats").insert(payload);
      if (error) return;
    }
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="grid gap-6">
            ...
            <div className="grid gap-2">
              <div className="text-sm font-medium">Ajouter un mandat</div>
              <Select onValueChange={(val) => addMandate(Number(val))}>
                <SelectTrigger className="w-64">
                  <SelectValue
                    placeholder={
                      available.length ? "Choisir…" : "Aucun mandat disponible"
                    }
                  />
                </SelectTrigger>
                <SelectContent>
            ...
            <div className="grid gap-4">
              ...
              {fields.map((f, i) => {
                const typeId = form.getValues(`mandates.${i}.mandat_type_id`);
                const mt = types.find((t) => t.id === typeId);
                const flagged =
                  form.getValues(`mandates.${i}._delete`) === true;
                    ...
                    >
                      ...
                      <div className="flex items-end gap-3">
                        <label className="flex items-center gap-2 text-sm">
                          <Checkbox
                            checked={!!flagged}
                            onCheckedChange={(c) => toggleDelete(i, c === true)}
                          />
                          Désactiver
                        </label>
                      </div>
